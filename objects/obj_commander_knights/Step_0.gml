...
if (abs(dx) > abs(dy)) {
    sprite_index = going_sides_corrupted_commander_knight;
    image_xscale = (dx < 0) ? 1 : -1;
} else if (abs(dy) > 0) {
    if (dy < 0) sprite_index = going_up_corrupted_commander_knight;
    else       sprite_index = going_down_corrupted_commander_knight;
}
