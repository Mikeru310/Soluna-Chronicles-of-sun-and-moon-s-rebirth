/// STEP EVENT — Side Quest Giver (Herbalist Lysa)
...
if (!marker_created)
{
    ...
    {
        if (parent_npc == other.id && quest_id == "side_quest_2")
            has_marker = true;
    }
    ...
    if (instance_exists(obj_player))
    {
        ...
        if (close && keyboard_check_pressed(ord("Z")))
        {
            if (!instance_exists(obj_textbox))
            {
                ...
                create_textbox("side_quest_2_after");
...
if (instance_exists(obj_player))
{
    ...
    if (pc && keyboard_check_pressed(ord("Z")) && interaction_cooldown <= 0)
    {
        ...
        if (global.side_quest_2_item_found && !global.side_quest_2_complete)
        {
            if (!instance_exists(obj_textbox))
            {
                text_x_offset = array_create(50,0);
                ...
                create_textbox("side_quest_2_complete_dialogue");
                obj_player.can_move = false;
            }

            global.side_quest_2_complete = true;
            scr_show_popup("Side Quest Complete!");


            // Mark as complete in the main quest list
            ...
            for (var i = 0; i < array_length(global.quest_data); i++)
            {
                if (global.quest_data[i][? "id"] == "side_quest_2")
                {
                    global.quest_data[i][? "description"] =
                        "Herbalist Lysa needed a rare fern sample.\n" +
                        "You found it near the river in Area 1-3.";
                }
            }
        ...
        && global.side_quest_2_given)
        {
            if (!instance_exists(obj_textbox))
            {
                ...
                create_textbox("side_quest_2_progress");
                obj_player.can_move = false;
            }
        ...
        if (!global.side_quest_2_given
        && !global.side_quest_2_started
        && !global.side_quest_2_item_found)
        {
            if (!instance_exists(obj_textbox))
            {
...
// ---------------------------------------------------------
if (global.side_quest_2_requested && !global.side_quest_2_given)
{
    scr_quest_add("side_quest_2",
        "Herbalist's Request",
        "Find the rare Fern Sample near the river in Area 1-3.",
        "Side Quest");

    scr_show_popup("New Side Quest Added!");

    global.side_quest_2_given     = true;
    global.side_quest_2_started   = true;
    global.side_quest_2_requested = false;
}