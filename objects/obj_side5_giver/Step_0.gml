/// STEP EVENT — Side Quest 5 Giver (Old Man Neher)
...
if (!marker_created)
{
    ...
    {
        if (parent_npc == other.id && quest_id == "side_quest_5")
            has_marker = true;
    }
    ...
    if (instance_exists(obj_player))
    {
        ...
        if (close && keyboard_check_pressed(ord("Z")))
        {
            if (!instance_exists(obj_textbox))
            {
                ...
                create_textbox("side_quest_5_after");
...
if (instance_exists(obj_player))
{
    ...
    if (pc && keyboard_check_pressed(ord("Z")) && interaction_cooldown <= 0)
    {
        ...
        if (global.side_quest_5_item_found && !global.side_quest_5_complete)
        {
            if (!instance_exists(obj_textbox))
            {
                text_x_offset = array_create(50,0);
                ...
                create_textbox("side_quest_5_complete_dialogue");
                obj_player.can_move = false;
            }

            global.side_quest_5_complete = true;
            scr_show_popup("Side Quest Complete!");



            // Mark quest complete in quest_data
            ...
            for (var i = 0; i < array_length(global.quest_data); i++)
            {
                if (global.quest_data[i][? "id"] == "side_quest_5")
                {
                    global.quest_data[i][? "description"] =
                        "Old Man Neher wishes to recover his late wife's ring.\nSearch the cemetery (Area 3) to find it.";
                }
            }
        ...
        && global.side_quest_5_given)
        {
            if (!instance_exists(obj_textbox))
            {
                ...
                create_textbox("side_quest_5_progress");
                obj_player.can_move = false;
            }
        ...
        if (!global.side_quest_5_given
        && !global.side_quest_5_started
        && !global.side_quest_5_item_found)
        {
            if (!instance_exists(obj_textbox))
            {
...
// ---------------------------------------------------------
if (global.side_quest_5_requested && !global.side_quest_5_given)
{
    scr_quest_add("side_quest_5",
        "Dying Wish",
        "Retrieve Neher's wife's ring from Cemetery Area 3.",
        "Side Quest");

    scr_show_popup("New Side Quest Added!");

    global.side_quest_5_given     = true;
    global.side_quest_5_started   = true;
    global.side_quest_5_requested = false;
}
